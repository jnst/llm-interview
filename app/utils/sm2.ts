...
export class SM2Algorithm {
   ...
   * @returns 新しい難易度係数
   */
  private calculateEaseFactor(quality: number, previousEaseFactor: number): number {
    // EF' = EF + (0.1 - (5 - q) * (0.08 + (5 - q) * 0.02))
    const newEaseFactor = previousEaseFactor + (0.1 - (5 - quality) * (0.08 + (5 - quality) * 0.02));
   ...
   * @returns 次回の復習間隔（日数）
   */
  private calculateInterval(reviewCount: number, previousInterval: number, easeFactor: number): number {
  ...
  public calculateNext(
    ...
    // 新しい難易度係数を計算
    const newEaseFactor = this.calculateEaseFactor(adjustedQuality, previousEaseFactor);
    ...
    if (adjustedQuality < this.config.qualityThreshold) {
      ...
      // 通常の間隔計算
      newInterval = this.calculateInterval(newReviewCount, previousInterval, newEaseFactor);
    }
  ...
  public createInitialProgress(interviewId: string): UserProgress {
    const now = new Date();
    const nextReviewDate = new Date();
    nextReviewDate.setDate(nextReviewDate.getDate() + this.config.initialIntervals[0]);
   ...
   */
  public static getMasteryLevel(progress: UserProgress): "初心者" | "学習中" | "習得済み" {
    if (progress.reviewCount === 0) return "初心者";
    if (progress.easeFactor >= 2.0 && progress.reviewCount >= 3) return "習得済み";
    return "学習中";
  }
   ...
   * @returns 復習予定かどうか
   */
  public static isDue(progress: UserProgress, date: Date = new Date()): boolean {
  ...
  public static isNew(progress: UserProgress | null): boolean {
    return progress === null || progress.reviewCount === 0;
  }
}
