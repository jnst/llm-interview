...
export class StudyManager {
   ...
   * @returns 新しい学習セッション
   */
  public startStudySession(interviews: Interview[], config: SessionStartConfig): StudySession {
    const progress = LocalStorageManager.getProgress();
    
    // 1. 復習対象カードを選択（nextReviewDate <= today）
    const reviewCards = this.getReviewCards(interviews, progress, config.categories, config.difficulties);
    
    // 2. 新規カードを選択（reviewCount === 0）
    const newCards = this.getNewCards(interviews, progress, config.categories, config.difficulties);
    ...
    if (config.includeNew && newCards.length > 0) {
      ...
      const newLimit = Math.min(remainingSlots, Math.floor(config.maxCards * 0.3)); // 新規30%
  ...
  private getReviewCards(
    ...
    return interviews
      .filter(interview => {
        // カテゴリーと難易度のフィルター
        if (categories.length > 0 && !categories.includes(interview.category)) return false;
        if (difficulties.length > 0 && !difficulties.includes(interview.difficulty)) return false;
   ...
   */
  public addReviewToSession(sessionId: string, reviewedInterview: ReviewedInterview): StudySession | null {
    ...
    const averageResponseTime = updatedReviews.reduce((sum, r) => sum + r.responseTime, 0) / totalAnswers;
  ...
  private updateProgressForReview(review: ReviewedInterview): void {
    const currentProgress = LocalStorageManager.getProgressForInterview(review.interviewId);
  ...
  public calculateSessionStats(session: StudySession): SessionStats {
    ...
    const qualityDistribution = reviewedInterviews.reduce((acc, review) => {
    ...
    return {
      ...
      averageResponseTime: reviewedInterviews.length > 0 ? 
        reviewedInterviews.reduce((sum, r) => sum + r.responseTime, 0) / reviewedInterviews.length : 0,
  ...
  public calculateStudyStats(interviews: Interview[]): StudyStats {
    ...
    // 新規カード
    const newToday = interviews.filter(i => SM2Algorithm.isNew(progress[i.id])).length;
    ...
    // カテゴリー別進捗
    const categoryProgress = this.calculateCategoryProgress(interviews, progress);
  ...
  private calculateCategoryProgress(
    ...
    const categories: Category[] = ["基礎概念", "アーキテクチャ", "学習手法", "応用技術", "評価指標", "実装技術", "倫理・社会的影響"];
    ...
    return categories.map(category => {
      ...
      const masteredCards = categoryProgressData.filter(p => 
        SM2Algorithm.getMasteryLevel(p) === "習得済み"
      ).length;
      
      const reviewedCards = categoryProgressData.filter(p => p.reviewCount > 0);
      const averageAccuracy = reviewedCards.length > 0 ?
        (reviewedCards.reduce((sum, p) => sum + (p.correctCount / p.reviewCount), 0) / reviewedCards.length) * 100 : 0;
   ...
   * @returns 復習予定カード数
   */
  public getDueCardsCount(interviews: Interview[], date: Date = new Date()): number {
    const progress = LocalStorageManager.getProgress();
   ...
   * @returns 学習可能なカード数
   */
  public getAvailableCardsCount(interviews: Interview[], config: Partial<SessionStartConfig>): number {
    const progress = LocalStorageManager.getProgress();
    const today = new Date();
    
    return interviews.filter(interview => {
      // カテゴリーフィルター
      if (config.categories && config.categories.length > 0 && !config.categories.includes(interview.category)) {
        return false;
      }
      
      // 難易度フィルター
      if (config.difficulties && config.difficulties.length > 0 && !config.difficulties.includes(interview.difficulty)) {
        return false;
      }
      ...
      // 復習カードを含む場合
      if (config.includeReview && userProgress && SM2Algorithm.isDue(userProgress, today)) {
        return true;
      }
