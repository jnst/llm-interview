...
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (action === "update_settings") {
    ...
    const cardsPerSession = parseInt(formData.get("cardsPerSession") as string);
    ...
    const autoFlipDelay = parseInt(formData.get("autoFlipDelay") as string);
  ...
  if (action === "export_data") {
    return json({ success: true, export: true, message: "データをエクスポートしました" });
  }

  if (action === "reset_progress") {
    return json({ success: true, reset: true, message: "学習記録をリセットしました" });
  }

  if (action === "reset_all") {
    return json({ success: true, resetAll: true, message: "すべてのデータをリセットしました" });
  }
  ...
  useEffect(() => {
    if (actionData?.success) {
      if (actionData.settings) {
        ...
        // テーマの適用
        document.documentElement.classList.toggle("dark", actionData.settings.theme === "dark");
      }
      
      if (actionData.export) {
  ...
  const handleImportFile = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (success) {
        ...
        document.documentElement.classList.toggle("dark", importedSettings.theme === "dark");
        alert("データをインポートしました");
      }
    } catch (error) {
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="max-w-2xl mx-auto space-y-6">
            ...
            <Card>
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  <div>
                    <label className="block text-sm font-medium text-text mb-2">テーマ</label>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => handleSettingsChange({ cardsPerSession: parseInt(e.target.value) })}
                      className="w-full"
                    />
                    <div className="flex justify-between text-xs text-gray-600 dark:text-gray-400 mt-1">
                      <span>5</span>
                      <span className="font-semibold text-primary">{settings.cardsPerSession}枚</span>
                      <span>50</span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    <label className="flex items-center space-x-3">
                      <input
                        ...
                        onChange={(e) => handleSettingsChange({ autoFlip: e.target.checked })}
                    ...
                    {settings.autoFlip && (
                      <div className="ml-6">
                        ...
                        <input
                          ...
                          onChange={(e) => handleSettingsChange({ autoFlipDelay: parseInt(e.target.value) })}
                          className="w-full"
                        />
                      </div>
                    )}
                  ...
                  <div className="space-y-3">
                    ...
                    <label className="flex items-center space-x-3">
                      <input
                        ...
                        className="rounded border-gray-300 text-primary focus:ring-primary"
                      />
                      <span className="text-text">キーボードショートカット</span>
                    </label>
                  </div>
                </div>
              </div>
          ...
          {settings.keyboardShortcuts && (
            <Card>
              <div className="space-y-4">
                <h3 className="text-lg font-semibold text-text">キーボードショートカット</h3>
